...
int sg16(int fd, int rw, struct ata_tf *tf, void *data, unsigned int data_bytes,
         unsigned int timeout_secs)
{
        ...
        io_hdr.dxfer_direction =
                data ? (rw ? SG_DXFER_TO_DEV : SG_DXFER_FROM_DEV) :
                       SG_DXFER_NONE;
        ...
        if (verbose) {
                fprintf(stderr,
                        "SG_IO: ATA_16 status=0x%x, host_status=0x%x, driver_status=0x%x\n",
        ...
        if (verbose)
                fprintf(stderr,
                        "      ATA_16 tf->status=0x%02x tf->error=0x%02x\n",
                        tf->status, tf->error);
        return 0;
}
