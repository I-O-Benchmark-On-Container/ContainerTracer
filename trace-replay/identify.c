...
#define ADV_PWR                                                                \
        91 /* current advanced power management level
...
#define STD_STBY                                                               \
        0x2000 /* 1=standard values supported (ATA);
...
#define MIN_STANDBY_TIMER                                                      \
        0x0001 /* 1=device specific standby timer value minimum */
...
#define MODE_MAX                                                               \
        7 /* bit definitions force udma <=7 (when
...
const char *minor_str[MINOR_MAX + 2] = {
        /* word 81 value: */
        "Unspecified", /* 0x0000	*/
...
void identify(__u16 *id_supplied)
{
	 ...
	 */
        if ((val[CONFIG] == STBY_NID_VAL) || (val[CONFIG] == STBY_ID_VAL) ||
            (val[CONFIG] == PWRD_NID_VAL) || (val[CONFIG] == PWRD_ID_VAL)) {
                like_std = 5;
                if ((val[CONFIG] == STBY_NID_VAL) ||
                    (val[CONFIG] == STBY_ID_VAL))
                        printf("powers-up in standby; SET FEATURES subcmd spins-up.\n");
                if (((val[CONFIG] == STBY_NID_VAL) ||
                     (val[CONFIG] == PWRD_NID_VAL)) &&
	 ...
	 * defined until ATA-3 & the CDROM std uses different words.) */
        ...
        if (dev == ATAPI_DEV) {
                ...
                if ((ll > 0x00FBFC10) && (!val[LCYLS])) {
                        ...
                        printf("\tsectors/track\t%u\t%u\n", val[LSECTS],
                               jj ? val[LSECTS_CUR] : 0);
                        if (jj)
                                bb = (__u64)val[LCYLS_CUR] * val[LHEADS_CUR] *
                                     val[LSECTS_CUR];
                        else
                                bb = (__u64)val[LCYLS] * val[LHEADS] *
                                     val[LSECTS];
                        printf("\t--\n");
                        if ((min_std == 1) &&
        ...
        if ((eqpt != CDROM) && (like_std > 3)) {
                int has_queuing = 0;
                if (transport == 1 ||
                    (val[SATA_CAP_0] && val[SATA_CAP_0] != 0xffff)) {
        ...
        if (dev == ATA_DEV) {
                ...
                if ((like_std < 3) && !(val[SECTOR_XFER_MAX] & SECTOR_XFER)) {
                        ...
                        printf("Max = %u\t",
                               val[SECTOR_XFER_MAX] & SECTOR_XFER);
                        printf("Current = ");
        ...
        if (!(val[CAPAB_0] & DMA_SUP)) {
                ...
                if ((dev == ATAPI_DEV) && (eqpt != CDROM) &&
                    (val[CAPAB_0] & DMA_IL_SUP))
	 ...
	 * than n (e.g. 3, 2, 1 and 0).  Print all the modes. */
        ...
        if (val[WHATS_VALID] & OK_W64_70) {
                if (val[PIO_NO_FLOW] || val[PIO_FLOW]) {
                        printf("\t     Cycle time:");
                        if (val[PIO_NO_FLOW])
                                printf(" no flow control=%uns",
                                       val[PIO_NO_FLOW]);
                        if (val[PIO_FLOW])
                                printf("  IORDY flow control=%uns",
                                       val[PIO_FLOW]);
                        printf("\n");
                }
        }
        ...
        if ((val[CMDS_SUPP_1] & VALID) == VALID_VAL) {
                ...
                print_features(val[CMDS_SUPP_0] & 0x7fff, val[CMDS_EN_0],
                               feat_word82_str);
                if ((val[CMDS_SUPP_1] & VALID) == VALID_VAL)
                        print_features(val[CMDS_SUPP_1] & 0x3fff,
                                       val[CMDS_EN_1], feat_word83_str);
                if ((val[CMDS_SUPP_2] & VALID) == VALID_VAL &&
                    (val[CMDS_EN_2] & VALID) == VALID_VAL) {
                        print_features(val[CMDS_SUPP_2] & 0x3fff,
                                       val[CMDS_EN_2], feat_word84_str);
                        if ((val[CMDS_SUPP_2] & 0x1800) == 0x1800 && val[116] &&
                            val[116] != 0xffff)
                ...
                if ((val[CMDS_SUPP_1] & VALID) == VALID_VAL &&
                    ...
                    (val[CMDS_EN_3] & VALID) == VALID_VAL)
                        print_features(val[CMDS_SUPP_3] & 0x3fff,
                                       val[CMDS_EN_3], feat_3_str);
                if (transport == 1 ||
                    (val[SATA_CAP_0] && val[SATA_CAP_0] != 0xffff))
                        print_features(val[SATA_CAP_0], val[SATA_CAP_0],
                                       cap_sata0_str);
                if (transport == 1 ||
                    (val[SATA_SUPP_0] && val[SATA_SUPP_0] != 0xffff))
                        print_features(val[SATA_SUPP_0], val[SATA_EN_0],
                                       feat_sata0_str);
                if (val[SCT_SUPP] & 0x1)
                        print_features(val[SCT_SUPP], val[SCT_SUPP] & 0x3f,
                                       feat_sct_str);
        }
        ...
        if (is_cfa) {
                ...
                if (max == 1 || max == 2) {
                        ...
                        for (mode = 1; mode <= max; ++mode) {
                                if (mode == selected)
                                        strcat(modes, "*");
                                sprintf(modes + strlen(modes), "pio%u ",
                                        mode + 4);
                        }
                }
                // CFA mdma3-4:
                ...
                if (max == 1 || max == 2) {
                        ...
                        for (mode = 1; mode <= max; ++mode) {
                                if (mode == selected)
                                        strcat(modes, "*");
                                sprintf(modes + strlen(modes), "mdma%u ",
                                        mode + 4);
                        }
                }
                if (val[164] & 0x8000) {
                        static const unsigned char io_times[4] = { 255, 120,
                                                                   100, 80 };
                        static const unsigned char mem_times[4] = { 250, 120,
                                                                    100, 80 };
                        max = val[164] & 7;
                        if (max <= 3)
