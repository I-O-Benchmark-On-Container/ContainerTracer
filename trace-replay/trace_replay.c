...
void wait_completion(struct thread_info_t *t_info, int cnt)
{
        ...
        while (1) {
                complete_count = io_getevents(t_info->io_ctx, 1, cnt,
                                              t_info->events, NULL);
        ...
        for (i = 0; i < nr_trace; i++) {
                ...
                if (detail) {
                        ...
                        if (traces[i].synthetic) {
                                ...
                                fprintf(fp,
                                        " Working Set = %d MB (utilization = %d) \n",
                ...
                pthread_spin_lock(&trace->trace_lock);
                temp_percent = (double)(trace->trace_io_cur +
                                        (trace->trace_io_cnt *
                                         (trace->trace_repeat_count - 1))) *
                               100 /
                               (trace->trace_io_cnt * trace->trace_repeat_num);
                pthread_spin_unlock(&trace->trace_lock);
        ...
        if (detail) {
                ...
                if (timeout) {
                        printf(" time = %.0fs (remaining = %.0fs) avgbw = %.3fMB/s curbw = %.3fMB/s Lat = %.6fs, time_diff = %.6fs ",
                               execution_time, timeout - execution_time, avg_bw,
                               cur_bw, latency, avg_time_diff);
...
int trace_io_put(char *line, struct trace_info_t *trace, int qdepth)
{
        ...
        if (sscanf(line, "%lf %d %d %d %x\n", &(io->arrival_time), &(io->devno),
                   &(io->blkno), &(io->bcount), &(io->flags)) != 5) {
                fprintf(stderr,
                        "Wrong number of arguments for I/O trace event type\n");
                fprintf(stderr, "line: %s", line);
                return -1;
        ...
        start = (trace->trace_io_cnt - qdepth > 0) ?
                        trace->trace_io_cnt - qdepth :
                        0;
        for (j = start; j < i; j++) {
                if ((trace->trace_buf[i]).devno ==
                    (trace->trace_buf[j]).devno) {
                        if ((trace->trace_buf[i]).flags ==
                                    (trace->trace_buf[j]).flags &&
                            ((trace->trace_buf[i]).blkno ==
                             ((trace->trace_buf[j]).blkno +
                              (trace->trace_buf[j]).bcount - 8))) {
...
#define MULTITHREAD_LOADER
                ...
                fprintf(result_fp,
                        " %d thread start part = %fGB size = %fGB (%llu, %llu pages)\n",
                        (int)i,
